    ...
    public class WaveController : MonoBehaviour
    {
        ...
        private IEnumerator EmitPlatesCoroutine()
        {
            ...
            for (var i = 0; i < plateNum; i++)
            {
                ...
                var speedVector = speed * new Vector3(left ? 1f : -1f, Random.Range(-0.2f, 0.2f), 0) *
                                 (float) Math.Pow(1.1, GameController.Instance.CurrentRound);
                
                GameController.Instance.ActionManager.RunAction(plate, new MoveTowardsDirectionAction(speedVector));
                if (!plate.GetComponent<PlateRetrieve>())
                {
                    plate.AddComponent<PlateRetrieve>();
                }
